...
///
/// * [`try_add_row`] and [`try_add_rows`] are fallible. They return a `Result` in the event of an overflow.
/// * [`add_row`] and [`add_rows`] do not return a result.
/// * [`row`] and [`rows`] are builder-style versions of `add_row` and `add_rows` that take and return `self` to allow
///   for method chaining.
///
/// The `rows` methods accept a list of lists of column indices (a nested iterator of `usize`).
///
/// ## Overflows
///
...
/// the number of rows.
///
/// [`try_add_row`]: Self::try_add_row
/// [`try_add_rows`]: Self::try_add_rows
...
impl HeaderBuilder {
    ...
    pub fn try_add_column(&mut self, column: Column) -> Result<(), MatrixOverflowError> {
    ...
    ///
    /// See also [`add_columns`][Self::add_columns] to add multiple columns at once from an iterator.
    ///
    /// # Panics
    ///
    ...
    ///
    /// See [`try_add_column`][Self::try_add_column] for a version without panics.
    pub fn add_column(&mut self, column: Column) {
        match self.try_add_column(column) {
            Ok(()) => {},
    ...
    /// The current implementation indexes columns with [`u32`], and uses [`u32::MAX`] as a sentinel value. This makes
    /// `u32::MAX - 1` the maximum number of columns allowed. This may change in the future.
    pub fn try_add_columns(&mut self, columns: impl IntoIterator<Item = Column>) -> Result<(), MatrixOverflowError> {
        ...
        self.nodes.reserve(est_len);
        self.col_headers.reserve(est_len);
    ...
    ///
    /// See also [`add_column`][Self::add_column] to add a single new column.
    ///
    /// # Panics
    ///
    /// This function will panic if the number of columns exceeds the maximum allowed limit. The current implementation
    ...
    /// number of columns allowed. This may change in the future.
    ///
    /// See [`try_add_columns`][Self::try_add_columns] for a version without panics.
    pub fn add_columns(&mut self, columns: impl IntoIterator<Item = Column>) {
        match self.try_add_columns(columns) {
            Ok(()) => {},
            Err(err) => handle_overflow(err),
        }
    }

    /// Builder-style version of [`Self::add_column`]. See there for details about panics.
    pub fn column(mut self, column: Column) -> Self {
        self.add_column(column);
        self
    }

    /// Builder-style version of [`Self::add_columns`]. See there for details about panics.
    ...
    pub fn finish_header(self) -> MatrixBuilder {
        let Self {
            mut nodes,
            col_headers,
            prev_required_idx,
...
impl MatrixBuilder {
    ...
    /// [`Self::num_columns`]).
    pub fn try_add_row(&mut self, columns: impl IntoIterator<Item = usize>) -> Result<(), MatrixOverflowError> {
    ...
    pub fn add_row(&mut self, columns: impl IntoIterator<Item = usize>) {
        match self.try_add_row(columns) {
            Ok(()) => {},
            Err(err) => handle_overflow(err),
        }
    }
    ...
    pub fn try_add_rows<R: IntoIterator<Item = usize>>(
        &mut self,
        ...
        // There isn't really a good way to reserve space for the nodes, at least that I can think of.
        self.row_headers.reserve(est_len);
    ...
    pub fn add_rows<R: IntoIterator<Item = usize>>(&mut self, rows: impl IntoIterator<Item = R>) {
        match self.try_add_rows(rows) {
            Ok(()) => {},
            Err(err) => handle_overflow(err),
        }
    }

    /// Builder-style version of [`Self::add_row`]. See there for details about panics.
    pub fn row(mut self, columns: impl IntoIterator<Item = usize>) -> Self {
        self.add_row(columns);
        self
    }

    /// Builder-style version of [`Self::add_rows`]. See there for details about panics.
    pub fn rows<R: IntoIterator<Item = usize>>(mut self, rows: impl IntoIterator<Item = R>) -> Self {
        self.add_rows(rows);
        self
    }

    /// Finish adding rows to the [`Matrix`] and build the final version.
    pub fn finish_matrix(self) -> Matrix {
        let Self {
            col_headers,
            row_headers,
            col_stack,
